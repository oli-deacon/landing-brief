...
export function AdaptiveNav() {
  ...
  return (
    <header className="sticky top-0 z-30 px-4 pt-[max(env(safe-area-inset-top),1rem)] sm:px-6 lg:px-8">
      <div className="nav-surface mx-auto flex w-full max-w-6xl items-center justify-between gap-3 rounded-[1.4rem] px-4 py-3 sm:px-5">
        ...
        <div className="flex items-center gap-2">
          <nav className="flex items-center gap-1 rounded-full border border-border-soft bg-white/3 p-1">
            {navItems.map((item) => (
              <NavLink
                key={item.to}
                to={item.to}
